...
//if($userservice->isLoggedOn() && ($user == "") && !isset($_GET['popup'])){
//}
...
// ADD A BOOKMARK
$saved = false;
$tplVars['formaction']  = createURL('bookmarks', $user . '?action=add'); 
$templatename = 'bookmarks.tpl';

if ($loggedon && isset($_POST['submitted'])) {

    if (!$_POST['title'] || !$_POST['address']) {
		//$_SESSION["messages"] = T_('Your bookmark must have a title and an address');
        $tplVars['error'] = T_('Your bookmark must have an Address and a Title');
        ...
        if ($bookmarkservice->bookmarkExists($address, $currentUserID)) {
			...
			$_SESSION["messages"] = "This bookmark already exists.";
        ...
        // If it's new, save it
			...
			$starred = trim($_POST['starred']);
            $saved = true;
            if ($bookmarkservice->addBookmark($address, $title, $description, $status, $categories, $starred)) {
                if (isset($_POST['popup'])) {
                    $tplVars['msg'] = '<script type="text/javascript">window.close();</script>';
                } else {
                    $tplVars['msg'] = T_('Bookmark saved');
					$_SESSION["messages"] = T_('Bookmark saved Succefully');
                }
            } else {
...
if ($templatename == 'editbookmark.tpl') {
	$_SESSION['pagename']='addbookmark';
	$_SESSION['addbookmarkactive'] = '1';
    if ($loggedon) {
        $tplVars['formaction']  = createURL('bookmarks', $user . '?action=add'); //createURL('bookmarks', $currentUsername);
...
//	$total_count_of_bookmarks =& $bookmarkservice->getUserBookmarksCount($start, $perpage, $userid, $cat, $terms, getSortOrder());
//echo "<pre>";
//print_r($bookmarks['total']);
//echo "</pre>";
