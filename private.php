...
$_SESSION['pagename']='private';
$_SESSION['privateactive'] = '1';
...
if ($templatename == 'editbookmark.tpl') {
    if ($loggedon) {
        $tplVars['formaction']  = createURL('bookmarks', $cUsername . '?action=add'); //createURL('bookmarks', $currentUsername);
    ...
    $tplVars['popCount'] = 30;
    $tplVars['sidebar_blocks'] = array('popular');
    ...
    $bookmarks =& $bookmarkservice->getBookmarksPrivate($start, $perpage, $userid, $cat, $terms, getSortOrder());
...
//echo "</pre>";
    ...
    if ($user == $currentUsername) {
        $title = T_('Private Bookmarks') . filter($catTitle);
    } else {
