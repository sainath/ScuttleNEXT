...
class UserService {
  ...
  var $sessionkey;
  var $cookiekey;
  var $cookietime = 2419200; // 2 weeks
  var $pagename='';
    ...
    function UserService(&$db) {
		...
		if(isset($_SESSION['pagename'])){
			$this->pagename = trim($_SESSION['pagename']);
		}
    }
    ...
    function getUserByEmail($useremail) {
        return $this->_getuser($this->getFieldName('email'), $useremail);
    }
    ...
    function getUserBookmarksCount($userid) {
        // returns the total count of bookamrks

		//$query='SELECT COUNT(*) AS total FROM sc_users AS U, sc_bookmarks AS B WHERE B.uId = U.uId AND ((B.bStatus = 2) OR (B.uId = '.intval($userid).')) AND B.uId = '.intval($userid);
		$query='SELECT COUNT(*) AS total FROM sc_users AS U, sc_bookmarks AS B WHERE B.uId = U.uId AND ((B.uId = '.intval($userid).')) AND B.uId = '.intval($userid);

        if (! ($dbresult =& $this->db->sql_query($query)) ) {
            message_die(GENERAL_ERROR, 'Could not get count of bookmarks', '', __LINE__, __FILE__, $query, $this->db);
            return false;
        }

		if (! ($row = & $this->db->sql_fetchrow($dbresult)) ) {
            message_die(GENERAL_ERROR, 'Could not get count of bookmarks1', '', __LINE__, __FILE__, $query, $this->db);
            return false;
        }
        return $row['total'];
    }

   function getUserTagsCount($userid) {
        // returns the total count of bookamrks
...
//		$query='SELECT COUNT(*) AS total FROM sc_users AS U, sc_bookmarks AS B WHERE B.uId = U.uId AND ((B.bStatus = 2) OR (B.uId = '.intval($userid).')) AND B.uId = '.intval($userid);
	...
	function getPage() { return $this->pagename; }
    // Properties
